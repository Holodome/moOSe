...
int arp_get_mac(u8 *ip_addr, u8 *mac_addr) {
    ...
    arp_send_request(ip_addr, mac_addr);

    int found = 0;
    size_t end = get_seconds() + 60;
    while (!found && get_seconds() < end)
...
void arp_send_request(u8 *ip_addr, u8 *mac_addr) {
    u8 frame[sizeof(struct arp_header)];

    struct arp_header *header = (struct arp_header *)frame;
    header->hw_type = bswap16(ETH_HW_TYPE);
    header->protocol_type = bswap16(ETH_TYPE_IPV4);
    header->hw_len = 6;
